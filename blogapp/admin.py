...
class PostAdmin(admin.ModelAdmin):
    list_display = ('title', 'author', 'date_to_publish', 'status')
    ...
    fieldsets = (
        ...
        ('Tags', {"fields": ('tags', )}),
        ('Status', {"fields": ('date_to_publish', )})
    )

    # custom list_display data
    def status(self, obj):
    ...
    def make_publish(self, request, queryset):
        ...
        of their actions
        """
        updated = queryset.update(date_to_publish=timezone.now())
    ...
    def make_draft(self, request, queryset):
        ...
        updated = queryset.update(date_to_publish=date)
        if updated == 1:
            msg = "1 post was"
        else:
            msg = F"{updated} posts were"
